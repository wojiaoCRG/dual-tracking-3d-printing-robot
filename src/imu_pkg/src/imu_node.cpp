...
{   
    ...
    else if(abs(abs(target_yaw) - abs(yaw)) <= 3 && (abs(diff_angle)< 270 || abs(diff_angle) >348))
    { 
        if (go_times == 3){
            //通过slam地图让小车回原点
            MoveBaseClient ac("move_base", true);//ac、goal对象的定义需要放在最前面第，不然会报错
    ...
    //直行时允许左右转微调角度
    if(abs(abs(target_yaw) - abs(yaw)) <=10 && (abs(diff_angle)< 270 || abs(diff_angle) >348)){     //(abs(diff_angle)< 270 || abs(diff_angle) >348)为了解决当target_yaw=-179~-130度时，yaw=140-130度，然后相差10度，进入直行微调
